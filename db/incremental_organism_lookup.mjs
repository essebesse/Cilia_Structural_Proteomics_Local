...
async function incrementalOrganismLookup() {
  const client = await db.connect();

  try {
    console.log('🚀 Starting INCREMENTAL organism lookup...');
    console.log('⚠️  This will ONLY update Unknown proteins, preserving all existing assignments\n');

    // Apply high-confidence rules first (only Unknown proteins)
    ...
    console.log(`\n🔍 Looking up ${unknownProteins.rows.length} Unknown proteins via UniProt API...`);

    if (unknownProteins.rows.length === 0) {
      console.log('✅ No proteins need organism lookup!');
      return;
    }
...
console.log('🧬 Incremental Organism Lookup Tool');
console.log('This tool ONLY updates Unknown proteins, preserving existing assignments\n');

incrementalOrganismLookup()
  .then(() => console.log('✅ Incremental organism lookup completed successfully'))
